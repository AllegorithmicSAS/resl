...
module.exports = function resl(config) {
  ...
  function loadXHR(request) {
    ...
    function onReadyStateChange() {
      ...
      loader.ready =
        (request.stream && !!asset) || loader.state === STATE_COMPLETE;
      notifyProgress();
    }
  ...
  // is correct
  var pending = Object.keys(manifest)
    .map(function (name) {
      ...
      function getString(prop, required, init) {
        ...
        if (typeof value !== "string") {
          raise(
            "invalid " + prop + ' for asset "' + name + '", must be a string'
          );
        }
        return value;
      }
      ...
      function getParseFunc(name, dflt) {
        if (name in request.parser) {
          ...
          if (typeof result !== "function") {
            raise(
              "invalid parser callback " + name + ' for asset "' + name + '"'
            );
          }
          return result;
        } else {
